...
if (isset($_SESSION['admin_username']) && $_SESSION['admin_username'] <> "" ) 
{
	...
	$countrylist = "";
	$rake = 0;
	$wuSQuery = "SELECT DATE_FORMAT(w.wu_mtcn_date,'%Y-%m-%d') AS dateSent, 
                     (w.wu_amount) AS usd_amount, wu_bonus_credit,
                     w.*, UPPER(u.user_username) AS username
               FROM westernunion w 
               LEFT JOIN users u
               ...
               WHERE w.wu_mtcn_no <> 'MANUALOAD'
                  ...
                  AND w.wu_mtcn_no <> 'MANAUALOAD'
                  AND w.wu_mtcn_no <> 'BNKTRANSFR'
                  AND w.wu_mtcn_no <> 'BANKTRNSFR'
                  AND w.wu_mtcn_no <> 'bank trans'
                  AND w.wu_mtcn_no <> 'VIRTUAL'
                  AND w.wu_mtcn_no <> 'ADJUSTMENT'
				  AND w.wu_mtcn_no <> 'BANK'
...
//		   $recId = $recId + 1;
         ...
         //$countrylist .= "<tr><td align='center'>$country_count</td><td align='right'>{$row["dateSent"]}</td><td align='right'>{$row["wu_mtcn_no"]}</td><td align='right'>$usd_amount</td><td align='left'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$sender</td></tr>"; 
			$countrylist .= "<tr><td align='right'>$recId</td><td align='right'>{$row["dateSent"]}</td><td align='right'>{$row["wu_mtcn_no"]}</td><td align='right'>$usd_amount</td><td align='left'>{$row["wu_bonus_credit"]}</td><td align='left'>$sender</td><td>{$row["username"]}</td></tr>"; 
		  }
...
<table  class="body-wrapper" width="1100"border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td width="800"  align="center" valign="top" class="content">
     ...
     <!--COntent -->		
    <div id="content">
    		<h2>Manual Loading via Western Union</h2>
         <p>
         <table border ="1" width="100%">
            <tr>
               <td width="50" align="center"><strong>#</strong></td> 
               <td align="center"><strong>Loaded</strong></td>
               <td align="center"><strong>MTCN</strong></td>
